...
projectPath = "~/Desktop/projectAI/BEC/openInsider/" 
BEC_getGraphs <- function( tradePurchaseFile, stopLossPct, takeProfitPct,
  ...
  for (irow in seq_len(nrow(tradePurchaseFile))){
    ...
    outputDF <- rbindlist(list(outputDF, tradeDF2), fill = T)
...
BEC_getGraphs(tradePurchaseFile, stopLossPct = 5, takeProfitPct = 5,
